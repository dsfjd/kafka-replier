...
@EmbeddedKafka
@DirtiesContext
@SpringBootTest(properties = { "stub-file-location=classpath:stub-examples/stub-files.json" })
class FileReplyIT {

    //todo unify all kafka tests
    @Autowired
        ...
        await().atMost(TEN_SECONDS)
                .untilAsserted(() ->
                                       assertThat(consumer.poll(ONE_SECOND))
                                               ...
                                               .containsExactly(tuple("output", "{\"example\": \"file-reply\"}"))
                );
    }
}
